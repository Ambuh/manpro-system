...
class theProLib{
    ...
    public function quinceMenu(){
		...
		if($this->positionHasPrivilege($this->ud->user_userType,72)|($this->ud->user_id==1))
		...
		$cont->generalTags('<div class="menu_item" id="m_22"><a href="#">Expenses</a><div class="at_icon" id="al_22"></div></div>');
		...
		$cont->generalTags('<div class="menu_item" id="m_21"><a href="#"> Income</a><div class="at_icon" id="al_21"></div></div>');
	...
	public function showScannedMp($id,$opt=0,$extn=0){
		...
		switch($opt){
			...
			case 7:
				//.$id.$extype
				$mt=header('Content-Type:image/'.$extn);
				$mt.=file_get_contents(dirname(__FILE__)."/../../mpFile/".$this->cmp->company_prefix."ufiles/comp-img-".$id);
				return $mt;
				break;
    ...
    public function requisitionMenu(){
		...
		//echo count($this->getMyActiveSite())
				...
				if(!$this->positionHasPrivilege($this->ud->user_userType,70))
                    $cont->generalTags('<div class="men_tab2" id="inMen_37" style="background:#076aa3;color:#ffffcc">Material Management</div>');
    ...
    public function assetInvestMenu(){
        ...
        $cont->generalTags('<div class="men_tab2" id="inMen_4">Graphical</div>');
	...
	public function inventoryMenu(){
		...
		$cont=new objectString;
          ...
          if(!$this->positionHasPrivilege($this->ud->user_userType,70) & $this->positionHasPrivilege($this->ud->user_userType,80))
            $cont->generalTags('<div class="men_tab2" id="inMen_44">View Main Store</div>');
		
			if($this->ud->user_userType==1 | $this->positionHasPrivilege($this->ud->user_userType,103) & !$this->positionHasPrivilege($this->ud->user_userType,70));
		    $cont->generalTags('<div class="men_tab2" style="background:#acf7aa" id="m_14">Store Managment</div>');
		  ...
		  if($this->positionHasPrivilege($this->ud->user_userType,57)|($this->ud->user_id==1))
            $cont->generalTags('<div class="men_tab2" id="inMen_10" style="background:#acf7aa;">+Receive Stock</div>');
	...
	public function equipmentMenu(){
		//<div class="a3-green men_tab2" id="inMen_98">View Requests</div>
		$cont=new objectString();
		
		
		
		if($this->positionHasPrivilege($this->ud->user_userType,77)&( $this->ud->user_userType !=1) & $this->positionHasPrivilege($this->ud->user_userType,70))
			$cont->generalTags("<div class='a3-right a3-blue a3-round a3-border a3-padding  men_tab2' id='inMen_99'> + Request Equiment</div>");
		
		if($this->positionHasPrivilege($this->ud->user_userType,78)|($this->ud->user_userType==1)|($this->ud->user_userType==5))
				$cont->generalTags('<div class="men_tab2" id="inMen_15" >View Equipment</div>');
			
		if($this->positionHasPrivilege($this->ud->user_userType,77) & $this->ud->user_userType !=1)
			    $cont->generalTags('<div class="men_tab2" id="inMen_18" style="background:#1e99e6;color:#fff;">Transfer Requests</div>');
			
		if($this->positionHasPrivilege($this->ud->user_userType,78)|($this->ud->user_userType==1)|($this->ud->user_userType==5))
				$cont->generalTags('<div class="men_tab2" id="inMen_17" style="background:#9df3b8;">+Add Equipment</div>');
	...
	public function officeProcurementMenu(){
		...
		if($this->positionHasPrivilege($this->ud->user_userType,22))		 
		 $cont->generalTags('<div class="men_tab2" id="inMen_31" >View Expenses</div>');
		   
		if($this->positionHasPrivilege($this->ud->user_userType,53))
		 $cont->generalTags('<div class="men_tab2" id="inMen_32" style="background:#acf7aa;">+ New Expense</div>');
    ...
    public function getDynamicPrivileges(){
		...
		return array(new name_value('<b>PROJECT MANAGEMENT</b>',-1,-1),new name_value('View Projects',2),new name_value('Create/Edit Project',-2),new name_value('Manage BOQ',71),new name_value('Generate Components Of Work',72),new name_value('Approve Components Of Work',102),new name_value('Assigned To One Project',70),new name_value("Manage Project Reports",701),new name_value('<b>REQUISITION MANAGEMENT</b>',-1,-1),new name_value('View Requisitions',4),new name_value('Approve Requisitions',-4,'slc'),new name_value('Generate Requisition',50),new name_value('Delete Requisition',65),new name_value('Upload Scanned Copy(Field Level)',51),new name_value('Upload Scanned Copy(Approved Copy)',60),new name_value('View Scanned Copy',61),new name_value('View Expense',22),new name_value('Approve Expense',52),new name_value('Generate Office Requisition',53),new name_value('<b>ACCOUNTS</b>',-1,-1),new name_value('Income & Expenses',21),new name_value('<b>PURCHASE & VERIFICATION</b>',-1,-1),new name_value('View Purchases',12),new name_value('Record Purchases',54),new name_value('View Overall Purchases',55),new name_value('View Overall Petty Cash  Purchases',56),new name_value('Price Verifications',13),new name_value('<b>INVENTORY MANAGEMENT</b>',-1,-1),new name_value('Issue Items',80),new name_value('Store Management',103),new name_value('View Inventory',5),new name_value('Receive Requisition Items',57),new name_value('View Overall Received',59),new name_value('<b>LABOUR MANAGEMENT</b>',-1,-1),new name_value('View Labour Records',10),new name_value('Upload Labour Records',17),new name_value('<b>REPORTS</b>',-1,-1),new name_value('Reports',7),new name_value("<b>PLANTS AND EQUIPMENT</b>",-1,-1),new name_value(" Make Equipment Request ",79),new name_value(" View plants and Equipment ",11),new name_value("Add Equipment ",78),new name_value(" Approve Equipment ",77),new name_value("Decline Equipment ",76) ,new name_value('<b>USER MANAGEMNT</b>',-1,-1),new name_value('View Users',8),new name_value('Create Users',-8),new name_value('<b>SETTINGS</b>',-1,-1),new name_value('Manage Settings',9),new name_value('Positions & Privileges',-39));
	...
	public function getLevelPrivileges(){
		...
		return array(new name_value('<b>REQUISITION PRIVILEGES</b>',-1),new name_value('Input Description',0),new name_value('Input Unit',2),new name_value('Input Rates',3),new name_value('Input Quantity',9),new name_value('Input Remarks',4),new name_value('Show All Columns',8),new name_value('View Rates',5));
	...
	public function searchInventory($whereclause){
		//ALTER TABLE `pr_equipment` ADD `equid_value` INT(10) NOT NULL DEFAULT '0' AFTER `date_recorded`;
		return '';
	}
	//---------------------------------------END SEARCH INVENTORY--------------------------------------
	//----------------------------------------EQUIPMENT MANAGER----------------------------------------
	public function addEquipment($reqCode,$descr="",$qty=1,$eqtype=0){
		
		return $this->db->insertQuery(array('equid_value','regcode','description','equipment_type','recorded_by','recorded_byname'),'pr_equipment',array("'".$qty."'","'".$reqCode."'","'".str_replace("'","''",$descr)."'",$eqtype,$this->ud->user_id,"'".str_replace("'","''",$this->ud->user_name)."'"));
	...
	public function getEquipment($whereClause="",$withSite=false){
		...
		if(!$withSite){
		  ...
		  $res=$this->db->selectQueryJoin(array('a.id','a.regcode','a.description','a.recorded_by','a.recorded_byname','a.equipment_type','a.date_recorded','b.site_id','sum(b.equid_value)'),array('pr_equipment a','pr_equiplocation b'),'where a.id=b.equip_id and b.isActive=1 '.$whereClause);
		}
		$results=array();
	...
	public function getTransferRequests($addedWhere="",$value=""){
		...
		$res=$this->db->selectQueryJoin(array('a.id ','b.id','a.source_site','a.id','b.site_title','c.id','c.regcode','c.description','a.byname','a.status',$value,'date_format(a.thedate,"%d/%b/%Y")'),array('pr_equiptransfer a','pr_sites b','pr_equipment c'),'where c.regcode=a.equip_id and b.id=a.dest_site '.$addedWhere);
	...
	public function getProjects($whereclause="",$rdFormat=false){
		...
		while($row=mysqli_fetch_row($res)){
			...
			$proj->project_budget=$row[7];
			$results[]=$proj;
		}
			
		return $results;
	}
	public function createProject($title,$manager,$start,$end,$location,$desc="",$budget="",$clerk=0){
		
		$id=$this->db->insertQuery(array('budget','project_name','project_manager','start_date','end_date','location','project_desc'),'pr_projects',array("'".$budget."'","'".$title."'",$manager,$this->dateFormatForDb($start),$this->dateFormatForDb($end),"'".$location."'","'".$desc."'"));
	...
	public function saveComponentImage($name,$id){

		$filename="comp-img-".$id.'_'.($this->listComponentImages($id));
																		 
		if(move_uploaded_file($_FILES[$name]['tmp_name'],dirname(__FILE__)."/../../mpFile/".$this->cmp->company_prefix."ufiles/".$filename)){
			$this->db->insertQuery(array(" type","content"),"pr_asset",array("1","'".$filename."'"));
		}
		
		
		
	}
	public function listComponentImages($id){
		
		$dir=scandir(dirname(__FILE__)."/../../mpFile/".$this->cmp->company_prefix."ufiles/");
		
		$files=0;
		
		for($i=0;$i<count($dir);$i++)
		    if(file_exists(dirname(__FILE__)."/../../mpFile/".$this->cmp->company_prefix.'ufiles/comp-img-'.$id.'_'.$i)){
			  if(is_file(dirname(__FILE__)."/../../mpFile/".$this->cmp->company_prefix.'ufiles/comp-img-'.$id.'_'.$i))
				$files+=1;
			}else{
				if(($dir[$i]!='..')&&($dir[$i]!='.')){
					break;
    ...
    public function saveRequisition($data,$sid,$rid=-1,$comp=-1){
		...
		if(count($req)>0)
			$reqno=count($req)+1;
		if($rid==-1){
		   $req_id=$this->db->insertQuery(array('project_id','requisition_no','req_date','byId','byName','site_id','level0','extType'),'pr_requisitions',array($sid,$reqno,'now()',$this->ud->user_id,"'".$this->ud->user_name."'",$sid,1,$comp));
		   $reqId=$req_id;
		}else{
			$req_id=$rid;
		...
		for($i=0;$i<count($data);$i++){
		  ...
		  if((trim($data[$i][0])=="")&(trim($data[$i][1])=="")&(trim($data[$i][2])=="")){}else{	  
		     $this->saveRequestItem($req_id,$sid,$sid,$data[$i][0],$reqI,$data[$i][2],0,0,$i);
		  }
		}
	...
	public function saveRequestItem($rId,$prid,$site_id,$desc,$qty,$unit,$rate=0,$amount=0,$lno=0){
	    ...
	    $as->checkMaterialUploads($desc,$unit);
	...
	public function getRequestItems($whereclause=""){
		...
		return $results;//req_siteId
	}
	public function getRequestItemPurchase($whereclause){
	    $req=$this->db->selectQuery(array("sum(qty),theRate"),'pr_purchaseditems',$whereclause);
    ...
    public function getRequestReceivedItems($whereclause){
	    $req=$this->db->selectQuery(array('sum(qty)'),'pr_receivedlogs',$whereclause);

	    $results=array();

	    while($row=mysqli_fetch_row($req)){
	        $results[]=array($row[0] !='' ? $row[0] : 0);
        }
        return $results;
    }
    public function verifyrequisitionPurchase($rid){
	    $mnArray=array();
	    /*  step 1 : get all the requision items and their quintities
	        step 2:check how much purcahse has been done per item
	    step 3:compute for excess values for each element and if the value is less allow purcahse to occur
	    step 4:return and array of purcahse and verified quantity
	    step 5 :create a switch case that determines the type of array that is gpoing to be returned -;if its is an just purchase,allowed valeues for purcashase*/
	...
	public function getReceivedItems($reqId,$other=""){
	    ...
	    $res=$this->db->selectQuery(array('project_id','site_id','description','sum(qty)','byid','byname','request_id','unit_type','pid','(select qty from '.$this->cmp->company_prefix.'pr_requisitionitems where description='.$this->cmp->company_prefix.'pr_receiveditems.description and request_id='.$reqId.' limit 1) as qty','(select theRate from '.$this->cmp->company_prefix.'pr_requisitionitems where description='.$this->cmp->company_prefix.'pr_receiveditems.description and request_id='.$reqId.' limit 1 ) as rate','site_id','(select sum(qty) from '.$this->cmp->company_prefix.'pr_purchaseditems  where  description='.$this->cmp->company_prefix.'pr_receiveditems.description and request_id='.$this->cmp->company_prefix.'pr_receiveditems.request_id)'),'pr_receiveditems','where request_id='.$reqId.' '.$other.' group by description asc');
	...
	public function getReceivedItems2($reqId,$other="",$withSite=false){
		$res=null;
		$hasTen=false;//print_r($reqId);
		if(is_numeric($reqId)){
		  $res=$this->db->selectQuery(array('project_id','site_id','description','qty','byid','byname','request_id','unit_type','pid','date_format(date_received,"%d-%b-%Y")','(select item_no from '.$this->cmp->company_prefix.'pr_requisitionitems where description='.$this->cmp->company_prefix.'pr_receivedlogs.description and request_id='.$reqId.' limit 1) as item_no'),'pr_receivedlogs','where request_id='.$reqId.' '.$other.'');
		  $hasTen=true;
		}else{
		   if($withSite){
				$res=$this->db->selectQueryJoin(array('a.project_id','b.site_id','a.description','a.qty','a.byid','a.byname','a.request_id','a.unit_type','a.pid','date_format(a.date_received,"%d-%b-%Y")','(select item_no from '.$this->cmp->company_prefix.'pr_requisitionitems where description=a.description and request_id=a.request_id limit 1) as item_no'),array('pr_receivedlogs a,pr_requisitions b'),'where a.request_id=b.id  '.$other);
			    $hasTen=true;
		   }else{
			   
  	            $res=$this->db->selectQuery(array('project_id','site_id','description','qty','byid','byname','request_id','unit_type','pid','date_format(date_received,"%d-%b-%Y")','(select item_no from '.$this->cmp->company_prefix.'pr_requisitionitems where description='.$this->cmp->company_prefix.'pr_receivedlogs.description and '.$this->cmp->company_prefix.'pr_receivedlogs.request_id=request_id limit 1) as item_no'),'pr_receivedlogs',$other);
			    $hasTen=true;
		   }
		}
		$results=array();
    ...
    public function getTotals($table,$field,$whereclause="",$case=-1){
		
		
		
		if($case !=-1){
		   // $whe='where a.request_id = b.request_id  '.$whereclause;
		    		    
		   $res=$this->db->selectQuery(array('sum(qty*theRate) as total'),'pr_purchaseditems',$whereclause);
	...
	public function createNewCompany($companyName="",$prefix="",&$theId=0){
		...
		return new name_value(true,System::successText("Company created successfully."),$theId);
	...
	//-----------------------------------------------WORK SCHEDULE----------------------------------------
	public function getWorkSchedule($whereclause="",$format="%d/%b/%Y",$status=0){

        $addedWhereclause=($status==0 ? " and is_component=0" : " " );
		
	   $res=$this->db->selectQuery(array('id','description','date_format(fromdate,"'.$format.'")','date_format(toDate,"'.$format.'")','datediff(toDate,fromdate)','(datediff(toDate,fromdate)-datediff(now(),fromdate))','projectId'),'pr_workschedule',$whereclause.$addedWhereclause);
	...
	public function getMaterialEstimates($whereclause="",$status=1){
		$as=System::shared("assist");
		$results=array();
		
		$res=$this->db->selectQuery(array('id','description','sum(qty)','component_id','unitType','(select sum(qty) from '.$this->cmp->company_prefix.'pr_issueditems where sch_id=component_id and '.$this->cmp->company_prefix.'pr_issueditems.description='.$this->cmp->company_prefix.'pr_materialestimates.description) as matissued','avg(cost)','component_is_child'),'pr_materialestimates',$whereclause." ");
		...
		while($row=mysqli_fetch_row($res)){
			...
			$est->mat_estCost=$row[6];
			$est->mat_actCost=($status==1 ? $as->getMaterialEstimateActualCost($row[3],$row[1]) : $as->getMaterialEstimateActualCost($row[3],$ww=(is_numeric($row[7]) ? count($this->getWorkSchedule("where id=".$row[7])) ? $this->getWorkSchedule("where id=".$row[7])[0]->wk_projectId  : 0 : 0  )) );
			$results[]=$est;
		}
	...
	public function getMaterialUsage($component_id){
		...
		$me=$this->getMaterialEstimates('where component_id='.$component_id.'  and (description <>"boq" and description <> 2 and description <> 3 and description  <> 4 and description <> 5) group by description');
	...
	//----------------------------------------------END WORK SCHEDULE-------------------------------------
	//---------------------------------------------------ADD ITMES----------------------------------------
	public function checkIssuedItem($description ,$qty,$id){
		
		$res=$this->db->selectQuery(array('sum(qty)'),'pr_receivedItems',"where description= '".$description."' and project_id=".$id." group by description limit 1");
		
		while($row =mysqli_fetch_row($res)){
			print_r($row[0]);
			return $typ=($row[0] > $qty) ? true : false;
		}
	}
	public function addIssuedItem($descrip,$qty,$unit,$id,$str='-2',$type){
		
		   
			if($type==1 & $this->checkIssuedItem($descrip,$qty,$id)){
				$this->db->insertQuery(array('description','qty','sch_id','store'),'pr_storeitems',array("'".$descrip."'","".$qty."","'".$id."'","'".$str."'"));
			}else{
				if($type !=1)
					$this->db->insertQuery(array('description','qty','sch_id','store'),'pr_issueditems',array("'".$descrip."'","'".$qty."'","'".$id."'","'".$str."'"));	
			}	
	}	
	//----------------------------------------------END ADD ITEMS------------------------------------------
	//----------------------------------------------STOCK BALANCES-----------------------------------------
	
	public function getStockBalances($store='-2',$projId=''){
		// 
		$addClause="";
		
		$addClause=" and project_id=".$projId;
		
		$exists=false;
		
		$pos=$this->getMyDynamicPrivileges(80);
		
		if(count($pos))
			$exists=true;
		
		if($this->positionHasPrivilege($this->ud->user_userType,80) &$this->positionHasPrivilege($this->ud->user_userType,70)){
			
            $us=System::shared("assist");
            
			/* connected to project site store 
			   */
			$position=0;
			$sa=$this->getSiteAssignments(" and a.user_id=".$this->ud->user_id.' and a.is_active=1');
			
            $us->getStores("where is_assigned=".$sa[0]->a_siteId);
			...
			$res=$this->db->selectQuery(array('description','sum(qty),unit','qty '),"pr_storeitems","where sch_id=".$sa[0]->a_siteId."  group by description");
			
		}else if($this->positionHasPrivilege($this->ud->user_userType,80) & !$this->positionHasPrivilege($this->ud->user_userType,70)){
			/* this is mainstore manager--
			is linked to issued items*/
			if($store=="-2"){
				//$res=$this->db->selectQuery(array('description','sum(qty),unit_type,(select sum(qty) from '.$this->cmp->company_prefix.'pr_storeitems where '.$this->cmp->company_prefix.'pr_receivedItems.description='.$this->cmp->company_prefix.'pr_storeitems.description and '.$this->cmp->company_prefix.'pr_receivedItems.store='.$this->cmp->company_prefix.'pr_storeitems.store limit 1) '),"pr_receivedItems"," where store = '".$store."' ".$addClause." group by description");
			
			}else{
				
				
				//$res=$this->db->selectQuery(array('description','sum(qty),unit,(select sum(qty) from '.$this->cmp->company_prefix.'pr_issueditems where description='.$this->cmp->company_prefix.'pr_storeitems.description limit 1) '),"pr_storeitems","where sch_id=".$projId."  group by description");
