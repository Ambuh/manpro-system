...
class HQuince{
    ...
    public function mainContainer(){
        ...
        $cont->generalTags('</div><section class="slidePopUp"></section>');
	...
	public function displayProject($pid=0){
		...
		for($i=0;$i<count($pr);$i++){		
		 ...
		 // $cont->generalTags("<div class='a3-left a3-padding a3-blue a3-hover-green a3-round a3-margin a3-button-face a3-pointer btn-project' id='gan_$pid'>Gann Chart</div>");

		  $cont->generalTags("<div class='a3-left a3-padding a3-blue a3-hover-green a3-round a3-margin a3-button-face a3-pointer btn-project' id='mem_$pid'>Project Members</div>");

		  $cont->generalTags("<div class='a3-left a3-padding a3-blue a3-hover-green a3-round a3-margin a3-button-face a3-pointer btn-project' id='email_$pid'>Send Reports</div>");
	...
	public function listPurchases($rid){
		...
		$list->setColumnNames(array('No.','Desription','Unit','Approved Qty','Approved Price','Purchase Price','Purchased Qty','Budget Variance',' ',' '));
		
		$list->setColumnWidths(array('5%','18%','8%','10%','10%','10%','10%','10%','10%','1%','4%'));
		...
		for($x=0;$x<count($pi);$x++){
			...
			$var =( ($pi[$x]->item_rate*$pi[$x]->item_qty) > ($pi[$x]->item_requested*$pi[$x]->item_theRate) ? " <div class='a3-left'>".number_format((($pi[$x]->item_rate*$pi[$x]->item_qty)-($pi[$x]->item_requested*$pi[$x]->item_theRate)),2)."</div><div class='a3-text-green a3-left'>More</div>" : "<div class='a3-left'>".number_format((($pi[$x]->item_requested*$pi[$x]->item_theRate)-($pi[$x]->item_rate*$pi[$x]->item_qty)),2)."</div><div class='a3-text-red a3-left'>Less</div>" );

			$item_qty+=$pi[$x]->item_qty;$app_qty +=$pi[$x]->item_requested;

            $list->addItem(array(($x+1),trim($pi[$x]->item_description),$pi[$x]->item_unit,$pi[$x]->item_requested." ".$pi[$x]->item_unit,number_format($pi[$x]->item_theRate,"2"),number_format($pi[$x]->item_rate),$pi[$x]->item_qty." ".$pi[$x]->item_unit, $var,' ','<div class="xpand" title="View Details" id="rq_'.$x.'_'.str_replace(' ','#',trim($pi[$x]->item_description)).'_'.$rid.'_'.$pi[$x]->item_theRate.'_'.$pi[$x]->item_requested.'"></div>',$share));
		...
		if(count($pi)){
			...
			$cont->generalTags("<nav class=' a3-full a3-left a3-padding a3-border-bottom'  ><b style='width:100px;float:left'>Budget Variance </b>".number_format( ($amountsArr-$requested),2)."</nav>");
	...
	public function purchaseList($id=0){
        ...
        for($i=0;$i<count($ri);$i++)
            $list->addItem(array($i+1,$ri[$i]->item_description,'Add Rate ','Add Qty ',$ri[$i]->item_unit,''));
	...
	public function showIssueItems(){
            ...
            */
			...
			$position=1;
             ...
             if($mSites !=null){
                 ...
                 $select->generalTags("<div id='label' class='a3-margin-left a3-margin-right'>Select Component</div><div class='qs_wrap'><select id='isMat_".$mSites->site_project."' class='aQuince_select proj' name='compHandler'>");
		...
		//'.$this->receiveList().'
		$cont->generalTags('<div class="q_row"><div id="ReqList"></div></div>');
	...
	public function receiveList($whereclause){
		...
		$mat=$this->QLib->getMaterialEstimates($whereclause." and (description <> 2 and description <> 'boq' and  description <> 3 and description <> 4 and description <> 5) group by description");
		
		for($i=0;$i<count($mat);$i++)
			$list->addItem(array(($i+1),$mat[$i]->mat_description,"click here ",$mat[$i]->mat_unitType));
	...
	public function receiveFromHq(){
		...
		if($this->positionHasPrivilege($this->ud->user_userType,57) & $this->positionHasPrivilege($this->ud->user_userType,70)){
			...
			$cont->generalTags('<div class="q_row" style="border-bottom:1px solid #ddd;padding-bottom:15px;"><div id="label">Requisition </div><div class="qs_wrap">'.$con->toString().'</div><div class="a3-left a3-margin-left a3-padding" style="margin-top:-.6%"><div id="label">Date:</div>'.$thDate->toString().'</div></div>');
...
//System::nameValueToSimpleArray($this->QLib->getPositionApprovalLevels($this->ud->user_userType),true);
	...
	public function showOverallList($theId=0){
		...
		$list->setColumnNames(array('Description','Amount','Description','Amount','%'));
		...
		$list->addItem(array(' ',' ','<b class="a3-text-orange">Cash Flow</b> ',number_format($income-($reqtotal+$office_req+$labour))));
	...
	public function newRequisition(){
		...
		if($this->QLib->positionHasPrivilege($this->ud->user_userType,70)){
		   ...
		   $cont->generalTags('<input type="hidden" value="'.$ss->site_id.'" id="sSite" />');
			...
			$cont->generalTags("<option value='-1'>Assign Component</option>");
...
//		/$cont->generalTags('</div>');
	...
	public function showComponent($id=0){
		...
		$cmp->setTagOptions(" autocomplete='off' ");
		...
		$from_date->setTagOptions(' autocomplete="off"  data-begin="'.$pr->project_start.'"  data-end="'.$pr->project_end.'"');
		...
		$toDate->setTagOptions('autocomplete="off"  data-begin="'.$pr->project_start.'"  data-end="'.$pr->project_end.'"');
	...
	public function listComponents($id){
		...
		$list->setColumnNames(array('No','Component','From Date','To Date','Duration','Remaining','%',(!$this->positionHasPrivilege($this->ud->user_userType,70) ? '<div class="a3-right">Cost</div>' : " "),' ',' '));
		
		$list->setColumnWidths(array('5%','20%','10%','10%','10%','10%','5%','15%','5%','5%','5%'));
		
		$ws=$this->QLib->getWorkSchedule('where projectId='.$id);
		
		for($i=0;$i<count($ws);$i++){
		
			$cost=$this->QLib->getMaterialEstimates("where component_id=".$ws[$i]->wk_id." and description='boq' ");



		   $list->addItem(array($i+1,$ws[$i]->wk_description,$ws[$i]->wk_frmDate,$ws[$i]->wk_toDate,($wk=$ws[$i]->wk_days>-1 ? $ws[$i]->wk_days : 0). ' days',($ww=$ws[$i]->wk_remaining>-1 ? $ws[$i]->wk_remaining : 0).' days',$this->QLib->getMaterialUsage($ws[$i]->wk_id).'%',(!$this->positionHasPrivilege($this->ud->user_userType,70) ? ($cost !=null ? "<div class='a3-right'>".number_format( ($cost[0]->mat_estCost*$cost[0]->mat_qty),2)."</div>" : "<div class='a3-right'>0.00</div>" ) : " "),'<div class="schSHw" id="sm_'.$ws[$i]->wk_id.'"></div>','<div class="edwk" id="ew_'.$ws[$i]->wk_id.'"></div>','<div class="lImage" id="uci_m-'.$ws[$i]->wk_id.'" title="View Upload Image"></div>'),'m-'.$ws[$i]->wk_id);
